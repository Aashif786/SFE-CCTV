...
class CorrelationEngine:
    ...
    def register_identity_event(self, event: IdentityEvent) -> None:
        ...
        print(f"[Identity] ⏳ Queued identity event for employee={event.employee_id}"
              f" gate={event.entry_gate} ts={event.timestamp.isoformat()}")

    # ------------------------------------------------------------------
    # Called by the camera pipeline when a new person/track appears
    # ------------------------------------------------------------------

    def on_new_track(self, camera_event: CameraEntryEvent) -> Optional[WorkerSession]:
        """
        ...
        with self._lock:
            ...
            if matched_event is None:
                print(f"[Identity] 🔍 New track {camera_event.track_id} on {camera_event.camera_id}"
                      f" — no matching identity event within {self._window}s window.")
                return None
    ...
    def _expire_stale(self, reference_time: datetime) -> None:
        ...
        for ev in self._pending:
            age = abs((reference_time - ev.timestamp).total_seconds())
            if age > self._window:
                ...
                print(f"[Identity] ⏰ Expired unmatched event for employee={ev.employee_id}"
                      f" (age={age:.1f}s > window={self._window}s)")
            else:
